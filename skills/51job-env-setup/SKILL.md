...
description: >
  ...
  全局安装 51job-cli（≥ 0.1.2，自动加 PATH），验证 51job --version
...
2. **安装**：默认 `npm install -g 51job-cli`（官方源，≥ 0.1.2）。仅当 `SJOB_CLI_SOURCE` 是 `git+...#ref` 时才 clone → `npm ci` → `npm run build` → `npm pack` → `npm install -g`。逻辑与 `install-dependencies.sh` 的 51job 分支一致，**不要另写一套**。
...
- 安装源默认 `51job-cli`（npm 官方源）。覆盖用环境变量 `SJOB_CLI_SOURCE`（钉版本 `51job-cli@0.1.2`，或 git 开发分支）。**不要改成未经验证的第三方包名**。
