...
> ### 🖥️ 给同事 / 新用户的分发入口
>
> 想把这套工作流交给别人（同事、HR、猎头）？先看 **[使用手册示例](docs/使用手册示例.md)**——按时间线手把手
> 教你装环境、初始化工作区、跑每日招聘，附常见问题排查和账号安全红线（哪些动作 AI 绝不会替你做）。
> 纯文档分发，不打包、不传数据，对方克隆仓库即可自己开始。
...
2. 运行下方「安装与使用」里的依赖安装脚本。脚本一并装三个平台的 CLI：Boss CLI 是
   ...
   [`51job-cli`](https://www.npmjs.com/package/51job-cli)（npm 官方源，≥ 0.1.2）。
